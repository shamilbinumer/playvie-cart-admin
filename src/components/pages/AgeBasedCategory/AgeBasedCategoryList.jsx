...
const CategoryList = () => {
  ...
  const handleEdit = (category) => {
      navigate(`/master/edit-age-based-category/${category.id}`, {
    state: {
      categoryData: {
    ...
    <>
      <BreadCrumb
        items={[
          ...
          { label: "Age Based Category List", path: "#" },
        ]}
      />
      <div className="px-2">
        <div className="mt-6">
          <PageHeader
            title="Age Based Category List"
            className="border-b border-gray-200 pb-4"
            actionButton={
              <AddButton
                title="Create New"
                onClick={() => navigate("/master/add-age-based-category")}
                disabled={loading}
              />
            }
          />
